...
class EncoderBlock(nn.Module):
    def __init__(self, ch_in, ch_out, depth=2, use_res=False, activation=nn.ReLU):
...
class DecoderBlock(nn.Module):
    ...
    Interpolate
    """
    def __init__(self, ch_in, ch_out, use_deconv=True, activation=nn.ReLU):
        super(DecoderBlock, self).__init__()
        if use_deconv:
            self.up = nn.Sequential(
                nn.ConvTranspose2d(ch_in, ch_out, kernel_size=2, stride=2) # use it
            )
        else:
            self.up = nn.Sequential(
